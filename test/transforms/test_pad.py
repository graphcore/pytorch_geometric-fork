...
@pytest.mark.parametrize(
    'node_pad_value',
    [None, 10, UniformPadding(12),
     AttrNamePadding({'x': 0})])
@pytest.mark.parametrize('edge_pad_value', [None, 11, UniformPadding(13)])
def test_pad_data_pad_values(node_pad_value, edge_pad_value):
    data = fake_data()
    original = copy.deepcopy(data)
    num_nodes = 32
    transform = Pad(max_num_nodes=32, node_pad_value=node_pad_value,
    ...
    EdgeTypePadding(
        {
            ...
            ('v1', 'e0', 'v0'): AttrNamePadding({'x': UniformPadding(-1.0)})
        })
])
